...
pkgs.testers.nixosTest {
  ...
  testScript = ''
    ...
    # ──── Phase 10: Verify installed system on disk ────
    # Inspect the installed NixOS system at /mnt to confirm it would boot
    # correctly with hearth-agent connected to the control plane.

    # Agent configuration was generated by the NixOS module
    enrollee.succeed("test -f /mnt/etc/hearth/agent.toml")
    enrollee.succeed("grep -q 'controlplane' /mnt/etc/hearth/agent.toml")

    # Machine identity survived nixos-install (written before install, not overwritten)
    enrollee.succeed("test -f /mnt/var/lib/hearth/machine-id")
    enrollee.succeed("test -f /mnt/var/lib/hearth/machine-token")

    # Bootloader installed on the target disk
    enrollee.succeed("test -d /mnt/boot/grub")

    # System profile exists and resolves to a valid NixOS system closure
    enrollee.succeed("test -e /mnt/nix/var/nix/profiles/system/activate")

    # The installed system has a working activate script
    enrollee.succeed("test -x /mnt/nix/var/nix/profiles/system/activate")

    enrollee.screenshot("07-system-verified")

    # ──── Phase 11: Verify structured logs ────
    enrollee.succeed("grep -q 'screen_transition' /tmp/hearth-enrollment.log")

    enrollee.screenshot("08-final")
  '';
}
