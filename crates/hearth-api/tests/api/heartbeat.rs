...
async fn heartbeat_updates_last_seen() {
    ...
    let (status, resp): (_, HeartbeatResponse) =
        send_json(&app, "POST", "/api/v1/heartbeat", Some(hb), None).await;
    assert_eq!(status, 200);

    // Verify machine's last_heartbeat is now set
    let uri = format!("/api/v1/machines/{}", machine.id);
    let (_, updated): (_, Machine) = send_json(&app, "GET", &uri, None, None).await;
...
async fn heartbeat_returns_target_closure() {
    ...
    let (_, _): (_, Machine) = send_json(&app, "PUT", &uri, Some(update), None).await;
...
async fn heartbeat_unknown_machine_returns_404() {
    ...
    let status = send_status(&app, "POST", "/api/v1/heartbeat", Some(hb), None).await;
