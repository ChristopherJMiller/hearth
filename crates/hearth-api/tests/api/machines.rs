...
async fn create_and_get_machine() {
    ...
    let (status, fetched): (_, Machine) = send_json(&app, "GET", &uri, None).await;
...
async fn list_machines() {
    ...
    let (status, machines): (_, Vec<Machine>) =
        send_json(&app, "GET", "/api/v1/machines", None).await;
    assert_eq!(status, 200);
...
async fn update_machine() {
    ...
    let (status, updated): (_, Machine) = send_json(&app, "PUT", &uri, Some(update)).await;
    assert_eq!(status, 200);
