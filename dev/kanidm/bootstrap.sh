...
# ---------------------------------------------------------------------------
# Step 0: Verify TLS cert exists (generated by `just setup` before docker-compose)
# ---------------------------------------------------------------------------
...
if [ ! -f "$CERT_FILE" ] || [ ! -f "$KEY_FILE" ]; then
    ...
    echo "       Run 'just setup' to generate it before running this script."
    exit 1
fi
...
set_person_password() {
    ...
    # Set the POSIX/unix password for PAM authentication via kanidm-unixd.
    # This is separate from the web credential — without it, unix login fails.
    local unix_pw="${USER_PASSWORDS[$name]:-$desired_password}"
    $C -X PUT "$KANIDM_URL/v1/person/$name/_unix/_credential" \
        -H "Authorization: Bearer $IDM_TOKEN" \
        -H "Content-Type: application/json" \
        -d "{\"value\":\"$unix_pw\"}" > /dev/null 2>&1 \
        && echo "    Set unix password for '$name'" \
        || echo "    Warning: could not set unix password for '$name'"
}
...
# Generate an API token. The response is a bare JSON string like "token...",
# so strip the surrounding quotes.
API_TOKEN_RAW=$($C -X POST "$KANIDM_URL/v1/service_account/hearth-api-svc/_api_token" \
    -H "Authorization: Bearer $ADMIN_TOKEN" \
    -H "Content-Type: application/json" \
    -d '{"label":"hearth-api","expiry":null}' 2>/dev/null)
API_TOKEN=$(echo "$API_TOKEN_RAW" | jq -r 'if type == "string" then . else empty end' 2>/dev/null || echo "")

if [ -n "$API_TOKEN" ]; then
    echo "    Generated API token for hearth-api-svc"
else
    echo "    WARNING: Could not generate API token (response: $API_TOKEN_RAW)"
fi

# Grant the service account permissions to manage persons and groups.
# Required for the identity sync to provision POSIX attributes.
echo ""
echo "==> Granting service account Kanidm privileges..."
for role in idm_people_admins idm_group_admins; do
    idm_post "/v1/group/$role/_attr/member" '["hearth-api-svc"]' > /dev/null 2>&1 || true
    echo "    Added hearth-api-svc to $role"
done
