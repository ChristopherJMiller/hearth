...
- [ ] **Push mechanism: control-plane → agent fast-path.** Today's loop (edit `home-modules/*.nix` → worker claim → nix evaluate flake tarball → build → sign → push to Attic → fan out to `user_environments` → 60s agent poll → fetch) is ~60-120s round-trip. Each hop hides bugs. **RFC: [`docs/rfc-001-push-fast-path.md`](docs/rfc-001-push-fast-path.md)**. **Dev push scaffold landed:** `AgentRequest::ApplyClosure` variant + agent handler (`handle_apply_closure`, gated by `HEARTH_ENABLE_DEV_PUSH=1` env var which `dev/fleet-vm.nix` sets and production agents leave unset) + `just push-user-env <user> <closure-path>` recipe (nix-copies via SSH:2222 then sends IPC via Python AF_UNIX client) + wire-format unit tests. **Still wanted (prod push):** SSE stream from agent → API so `complete_user_env_build` notifies affected agents in ~1s instead of the 60s poll. Out of scope: replacing Attic for content delivery, removing the build worker queue.
