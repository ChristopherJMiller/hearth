...
- [ ] **LibreOffice Addons.xcu "unknown node m1" warning.** The VM test surfaces a benign-looking config warning on every unopkg add/list: `configmgr: unknown node "m1" in "...Addons.xcu"`. The `oor:name="m1"` child nodes under `OfficeToolBar/com.hearth.office.toolbar` don't match the schema LO expects — toolbar item slots should follow a different naming/wrapping pattern. The toolbar still renders, so this is a polish issue. Reference the LO 7.x Addons.xcu schema docs and the upstream `extras/source/templates/extension/` examples.
