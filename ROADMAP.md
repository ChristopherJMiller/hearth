...
- [ ] **Nextcloud Bookmarks in Helm bootstrap:** The Bookmarks app is installed in the dev bootstrap script (`dev/nextcloud/bootstrap.sh`) but the production Helm Nextcloud bootstrap job (`templates/nextcloud/job-bootstrap.yaml`) should also install it so Floccus bookmark sync works in production deployments.
