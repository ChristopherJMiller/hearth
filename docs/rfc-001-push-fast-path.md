...
3. ~~Add the SSE endpoint + broadcast plumbing on the API. Agent opts in via config (`push.enabled = false` initially).~~ **Done.** Endpoint: `GET /api/v1/machines/{id}/events` in `crates/hearth-api/src/routes/machine_events.rs`, gated by `MachineIdentity` with a path-id-must-match-token check. Per-machine `tokio::broadcast` registry in `crates/hearth-api/src/events.rs::EventBus`, lazy-allocated on first subscribe. Cross-replica fan-out via Postgres `LISTEN/NOTIFY` on the `hearth_machine_events` channel — see `events::run_listener` (spawned in `crates/hearth-api/src/main.rs`) and the `pg_notify` calls added to `repo::update_machine` and `repo::complete_user_env_build`. Agent-side SSE consumer in `crates/hearth-agent/src/event_stream.rs` with capped exponential backoff (1s → 30s, `backoff_for`) and a `PushState` that the poll loop selects on. `agent.push.enabled` defaults to false in `crates/hearth-common/src/config.rs::PushSettings`. Tests: `crates/hearth-api/tests/api/events.rs` (auth boundary + `complete_user_env_build` → `pg_notify` → `LISTEN` chain) and `crates/hearth-agent/src/event_stream.rs::tests` (backoff function + SSE frame parser).
4. ~~Flip `push.enabled = true` in the default agent config once observed reconnect-storm metrics look sane on the dev fleet.~~ **Done (dev only).** `services.hearth.agent.push.enable = true` in `dev/fleet-vm.nix`. Production fleets still get the secure default — opt in by setting the same option on the NixOS host. Module options live in `modules/agent.nix::services.hearth.agent.push.{enable, pushedPollInterval}`.
5. ~~Drop poll cadence to 300s under healthy stream; ship.~~ **Done.** `PushState::is_healthy` drives the cadence pick in `crates/hearth-agent/src/poller.rs::run_poll_loop`: `pushed_poll_interval_secs` (default 300s) while connected, `poll_interval_secs` (default 60s) while down. `PushSettings::pushed_poll_interval_secs` and the matching `services.hearth.agent.push.pushedPollInterval` module option allow per-fleet tuning.
